...
export function Dashboard({ onPlayTrack, tracks, artists, albums, playlists }: DashboardProps) {
  const stats = [
    { title: 'Всего треков', value: tracks.length.toString(), icon: Music, color: 'from-primary to-purple-400' },
    { title: 'Исполнителей', value: artists.length.toString(), icon: User, color: 'from-secondary to-blue-400' },
    { title: 'Альбомов', value: albums.length.toString(), icon: Album, color: 'from-purple-500 to-pink-400' },
    { title: 'Плейлистов', value: playlists.length.toString(), icon: ListMusic, color: 'from-blue-500 to-cyan-400' },
  ]
  ...
  const genreStats = [
    { genre: 'Electronic', percentage: 35, count: Math.floor(tracks.length * 0.35) },
    { genre: 'Rock', percentage: 28, count: Math.floor(tracks.length * 0.28) },
    { genre: 'Jazz', percentage: 18, count: Math.floor(tracks.length * 0.18) },
    { genre: 'Classical', percentage: 12, count: Math.floor(tracks.length * 0.12) },
    { genre: 'Hip-Hop', percentage: 7, count: Math.floor(tracks.length * 0.07) },
  ]
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-4">
              {recentTracks.map((track) => (
                  <div key={track.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer group">
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 bg-muted/30 rounded-lg">
                <div className="text-2xl font-bold text-secondary">3.2</div>
